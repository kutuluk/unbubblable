...
class Player extends Unit {
  ...
  animateCamera(multiplier) {
    ...
    const direction = this.oY.clone().applyAxisAngle(this.oZ, rotation);
    // Рассчитываем высоту камеры в этом фрейме
    const height = this.camHeight + this.camMotion * multiplier;

    // Перемещаем камеру
    const camPos = this.mesh.position.clone().add(direction.multiplyScalar(height / -2));
