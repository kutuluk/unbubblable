...
var ticker, connect, controller;

var log = new Log();

var player = new Player();
var playerEcho = new Player();
var playerTempEcho = new Player();

function Connect(ping) {
...
function init() {
	...
	*/
	...
	var meshTex = new THREE.Mesh(geometryTex, atlasMap);
	...
	var meshDec = new THREE.Mesh(geometryDec, transMaterial);
...
function render() {
	...
	//	log.appendText(JSON.stringify(this.rotation));
			...
			var height = camHeight + camMotion;

			// Перемещаем камеру
			var camDistance = height / -2;
			var camPos = new THREE.Vector3();
			camPos.addVectors(new THREE.Vector3(), new THREE.Vector3().multiplyScalar(camDistance));
