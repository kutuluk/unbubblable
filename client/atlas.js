...
class Atlas {
  constructor(cols, rows, resolution, src) {
    ...
    const uOff = 1 / this.cols / (this.resolution + 2);
    const vOff = 1 / this.rows / (this.resolution + 2);
    ...
    img.onload = () => {
      ...
      for (let y = 0; y < this.rows; y++) {
        for (let x = 0; x < this.cols; x++) {
          ...
          const sx = x * this.resolution;
          const sy = y * this.resolution;
          // Начальная позиция верхнего левого пиксела тайла в новой текстуре
          const dx = x * (this.resolution + 2);
          const dy = y * (this.resolution + 2);
          ...
          ctx.drawImage(
            img,
            sx + this.resolution - 1,
            sy,
            1,
            this.resolution,
            dx + this.resolution + 1,
            dy + 1,
            1,
            this.resolution,
          );
    ...
    for (let i = 0; i < this.cols * this.rows; i++) {
      ...
      const v = Math.floor(i / this.cols);
      const u = i - v * this.cols;
